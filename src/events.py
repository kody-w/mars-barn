...
def aggregate_effects(active_events: List[dict]) -> dict:
    ...
    Reads all 12 effect keys generated by _create_event().
    Downstream consumers opt into the keys they need.
    """
    combined = {
    ...
    for event in active_events:
        ...
        if "terrain_damage" in effects:
            combined["terrain_damage"] += effects["terrain_damage"]
        if "pressure_breach" in effects:
            combined["pressure_breach"] = max(combined["pressure_breach"], effects["pressure_breach"])
        if "radiation_multiplier" in effects:
            combined["radiation_multiplier"] *= effects["radiation_multiplier"]
        if "electronics_risk" in effects:
            combined["electronics_risk"] = max(combined["electronics_risk"], effects["electronics_risk"])
        if "solar_boost" in effects:
            combined["solar_boost"] *= effects["solar_boost"]
        if "solar_panel_cleaning" in effects:
            combined["solar_panel_cleaning"] += effects["solar_panel_cleaning"]
        if "wind_speed_ms" in effects:
            combined["wind_speed_ms"] = max(combined["wind_speed_ms"], effects["wind_speed_ms"])
        if "capacity_reduction" in effects:
            combined["capacity_reduction"] = max(combined["capacity_reduction"], effects["capacity_reduction"])
        if "failed_system" in effects:
            combined["failed_system"] = effects["failed_system"]
    return combined
